+function () {
	...
	const store = new Vuex.Store({
		...
		mutations: {
			...
			updateNote(state, p) {
				let id = state.notes.length - p.id;
				state.notes[id].text = p.text;
			},
	...
	Vue.component('todo', {
		...
		methods: {
			...
			updateNote() {
				...
				if (!txt) {return;}
				let params = {id: this.docId, text: txt};
				store.commit('updateNote', params);
